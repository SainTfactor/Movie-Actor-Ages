    ...
    .done(function(data) {
        ...
        for(var i = 0; i < $(html).find("div.theater").length; i++)
        {
            ...
            for(var j = 0; j < $(theater).find(".showtimes .show_left .movie").length; j++)
            {
                ...
                if ($.inArray($(movie).find(".name a")[0].innerHTML, arrayOfMovies) == -1) {
                    $('<div>', { text: $(movie).find(".name a")[0].innerHTML, id: $(movie).find(".name a")[0].innerHTML }).appendTo(document.body);
            ...
            $.getJSON(service_url + '?callback=?', {query:JSON.stringify(query)}, function(response) {
                if (response.result.length != 0) {
                    ...
                    */
                    ...
                    $("#" + value).html($("#" + value).html() + " - " + JSON.stringify(response.result[0]["/film/film/starring"]));

                    //arrayOfActors[arrayOfActors.length] = [response.result[0]["name"], JSON.stringify(response.result[0]["/film/film/starring"])];
                }
                else {
